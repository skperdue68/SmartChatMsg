...
function SmartChatMsg:HasEndingPhraseBeforeTime(sourceText, timeMatch)
    ...
    local startPos = source:find(target, 1, true)
    if not startPos or startPos <= 1 then
        return false
    end

    local before = source:sub(1, startPos - 1)
    local endingPatterns = {
        "%f[%a]until%f[%A]",
        "%f[%a]run%s+until%f[%A]",
        "%f[%a]runs%s+until%f[%A]",
        "%f[%a]ending%s+at%f[%A]",
        "%f[%a]end%s+at%f[%A]",
        "%f[%a]ends%s+at%f[%A]",
        "%f[%a]ending%f[%A]",
...
function SmartChatMsg:GetApproximateCountdownTextFromSeconds(diffSeconds, sourceText, timeMatch)
    ...
    if exactMinutes < 15 then
        return self:PluralizeTimeUnit(exactMinutes, "minute", "minutes")
    end

    local roundedMinutes = self:RoundToNearestQuarterHour(safeSeconds / 60)
    ...
    local parts = {}
    if days > 0 then
        table.insert(parts, self:PluralizeTimeUnit(days, "day", "days"))
    end
    if hours > 0 then
        table.insert(parts, self:PluralizeTimeUnit(hours, "hour", "hours"))
    end
    if minutes > 0 then
        table.insert(parts, self:PluralizeTimeUnit(minutes, "minute", "minutes"))
    end
...
function SmartChatMsg:GetLocalTimezoneDisplayName(epochSeconds)
    local timezoneName = self:Trim(os.date("%Z", tonumber(epochSeconds) or os.time()) or "")
    if timezoneName ~= "" then
        return timezoneName
    end

    local offset = self:GetLocalUtcOffsetHours(epochSeconds)
    local isDst = os.date("*t", tonumber(epochSeconds) or os.time()).isdst == true
    local lookup = {
        [-8] = "PST",
...
function SmartChatMsg:FindEmbeddedTimeDetails(text)
    ...
    local function isTokenBoundary(posBefore, posAfter)
        local beforeChar = posBefore >= 1 and lowerSource:sub(posBefore, posBefore) or ""
        local afterChar = posAfter <= #lowerSource and lowerSource:sub(posAfter, posAfter) or ""
        local beforeOk = beforeChar == "" or not beforeChar:match("[%a]")
        local afterOk = afterChar == "" or not afterChar:match("[%a]")
        return beforeOk and afterOk
    end

    local function trySnippetBeforeTimezone(token, tokenStart, tokenEnd)
        local snippetStart = math.max(1, tokenStart - 24)
        local snippet = lowerSource:sub(snippetStart, tokenStart - 1)
        local patterns = {
            {
                pattern = "(%d%d?)%s*:%s*(%d%d)%s*([ap])%.?%s*%.?m?%s*$",
                name = "12h_with_minutes_tz_near_token",
            ...
            {
                pattern = "(%d%d?)%s*([ap])%.?%s*%.?m?%s*$",
                name = "12h_hour_only_tz_near_token",
            ...
            {
                pattern = "(%d)(%d%d)%s*([ap])%.?%s*%.?m?%s*$",
                name = "12h_compact_3digit_tz_near_token",
                transform = function(h, m, ap)
                    local meridiem = (ap == "a") and "AM" or "PM"
                    local hour = self:NormalizeExtractedHour(h, meridiem)
                    local minute = tonumber(m)
            ...
            {
                pattern = "(%d%d)(%d%d)%s*([ap])%.?%s*%.?m?%s*$",
                name = "12h_compact_4digit_tz_near_token",
                transform = function(h, m, ap)
                    local meridiem = (ap == "a") and "AM" or "PM"
                    local hour = self:NormalizeExtractedHour(h, meridiem)
                    local minute = tonumber(m)
            ...
            },
            {
                ...
                name = "24h_with_minutes_tz_near_token",
                transform = function(h, m)
        ...
        for _, entry in ipairs(patterns) do
            local a, b, c = snippet:match(entry.pattern)
            if a then
                local hour, minute = entry.transform(a, b, c)
                if hour ~= nil and minute ~= nil then
                    local originalMatch = self:SliceOriginalByLowerPositions(source, sourceIndexMap, snippetStart, tokenEnd)
                    local detectedMatch, detectedHour, detectedMinute, detectedTimezone = self:TryReturnDetectedTime(source, originalMatch, hour, minute, token, entry.name)
    ...
    for _, token in ipairs(self:GetSupportedTimezoneTokens()) do
        ...
        while searchStart <= #lowerSource do
            ...
            if isTokenBoundary(tokenStart - 1, tokenEnd + 1) then
    ...
    local detectedMatch, detectedHour, detectedMinute

    detectedMatch, detectedHour, detectedMinute = tryPattern("(%d%d?)%s*:%s*(%d%d)%s*([ap])%.?%s*%.?m?%f[%A]", "12h_with_minutes_local", nil,
        function(hourText, minuteText, ap)
            local meridiem = (ap == "a") and "AM" or "PM"
            local hour = self:NormalizeExtractedHour(hourText, meridiem)
            local minute = tonumber(minuteText)
            if hour and minute and minute >= 0 and minute <= 59 then return hour, minute end
            return nil, nil
        end)
    if detectedMatch then return detectedMatch, detectedHour, detectedMinute, nil end

    detectedMatch, detectedHour, detectedMinute = tryPattern("(%d%d?)%s*([ap])%.?%s*%.?m?%f[%A]", "12h_hour_only_local", nil,
        function(hourText, ap)
            local meridiem = (ap == "a") and "AM" or "PM"
            local hour = self:NormalizeExtractedHour(hourText, meridiem)
            if hour then return hour, 0 end
            return nil, nil
    ...
    detectedMatch, detectedHour, detectedMinute = tryPattern("(%d)(%d%d)%s*([ap])%.?%s*%.?m?%f[%A]", "12h_compact_3digit_local", nil,
        function(hourText, minuteText, ap)
            local meridiem = (ap == "a") and "AM" or "PM"
            local hour = self:NormalizeExtractedHour(hourText, meridiem)
            local minute = tonumber(minuteText)
    ...
    detectedMatch, detectedHour, detectedMinute = tryPattern("(%d%d)(%d%d)%s*([ap])%.?%s*%.?m?%f[%A]", "12h_compact_4digit_local", nil,
        function(hourText, minuteText, ap)
            local meridiem = (ap == "a") and "AM" or "PM"
            local hour = self:NormalizeExtractedHour(hourText, meridiem)
            local minute = tonumber(minuteText)
    ...
    detectedMatch, detectedHour, detectedMinute = tryPattern("(%d%d?)%s*:%s*(%d%d)%f[%A]", "24h_with_minutes_local", nil,
        function(hourText, minuteText)
...
function SmartChatMsg:GetEmbeddedDayOffset(text, nowEpoch)
    local source = zo_strlower(tostring(text or ""))
    local now = os.date("*t", tonumber(nowEpoch) or os.time())

    if source:find("%f[%a]tomorrow%f[%A]") then
        return 1
    end

    if source:find("%f[%a]today%f[%A]") or source:find("%f[%a]tonight%f[%A]") then
        return 0
    end
    ...
    for _, pattern in ipairs(patterns) do
        local matchedDay = source:match(pattern)
        if matchedDay then
            local targetWday = self:GetWeekdayIndexByName(matchedDay)
            if targetWday then
...
function SmartChatMsg:GetCountdownUntilEmbeddedTimeText(text)
    local hour, minute, sourceTz = self:ExtractEmbeddedTimeParts(text)
    if not hour then
    ...
    local dayOffset = self:GetEmbeddedDayOffset(text, nowEpoch)
    if dayOffset == nil then
        dayOffset = 0
    end

    local targetEpoch = os.time({
        year = nowLocal.year,
    ...
    if dayOffset == 0 and targetEpoch <= nowEpoch then
        targetEpoch = targetEpoch + (24 * 60 * 60)
    elseif dayOffset > 0 and targetEpoch <= nowEpoch then
        targetEpoch = targetEpoch + (7 * 24 * 60 * 60)
    end
    ...
    self:DebugLog(string.format(
        "Countdown debug: countdown computed=%s nowEpoch=%s targetEpoch=%s localOffset=%s sourceOffset=%s dayOffset=%s",
...
function SmartChatMsg:InsertCountdownIntoMessageText(text)
    ...
    local countdownText = self:GetCountdownUntilEmbeddedTimeText(source)
    ...
    local insertionText = timeMatch
    if not sourceTz or self:Trim(sourceTz) == "" then
        insertionText = timeMatch .. " " .. self:GetLocalTimezoneDisplayName()
    end

    local updatedText = source:gsub(self:EscapeLuaPattern(timeMatch), insertionText .. " (" .. countdownText .. ")", 1)
